<!-- Banner animado -->
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&pause=1000&color=36BCF7&center=true&vCenter=true&width=720&lines=Hola%2C+soy+Emilio+Araya+Ordo%C3%B1ez;Estudiante+de+Ingenier%C3%ADa+en+Inform%C3%A1tica;Backend+%7C+Linux+%7C+Cybersecurity;Bienvenido+a+mi+perfil+de+GitHub" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Backend-Developer-blue?style=for-the-badge&logo=springboot" />
  <img src="https://img.shields.io/badge/DevOps-Enthusiast-FF6F00?style=for-the-badge&logo=docker" />
  <img src="https://img.shields.io/badge/Linux-Arch%20Linux-1793D1?style=for-the-badge&logo=arch-linux&logoColor=white" />
  <img src="https://img.shields.io/badge/Cloud-AWS%20EC2-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white" />
</p>

<p align="center">
  <a href="mailto:emiliolokillo611@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contactar-D14836?style=flat-square&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/emilio-araya" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-emilio--araya-181717?style=flat-square&logo=github" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=emilio-araya&label=Visitas&color=0e75b6&style=flat-square" alt="visitas" />
</p>

---

## 👋 About me

Soy **Estudiante de Ingeniería en Informática** apasionado por el desarrollo **backend**, la administración de sistemas **Linux** y los fundamentos de **ciberseguridad**.

Me especializo en construir soluciones reales: desde microservicios desplegados en la nube hasta entender cómo funciona el sistema operativo por dentro. Mi enfoque combina **código limpio**, **arquitectura robusta** y **automatización DevOps**.

🔹 Me encanta aprender construyendo cosas útiles que resuelvan problemas reales.  
🔹 Siempre abierto a **colaborar en proyectos open source** o discutir ideas técnicas.  
🔹 ✔ Pregúntame cualquier cosa, ¡estoy feliz de ayudar!

📍 **Actualmente en Chile** 🇨🇱

---

## 🚀 Proyecto Destacado

<table>
  <tr>
    <td width="50%" valign="top">
      <h3 align="center">🐳 Innovatech Chile - Plataforma DevOps</h3>
      <p align="center">
        <a href="https://github.com/emilio-araya/proyecto-innovatech" target="_blank">
          <img src="https://img.shields.io/badge/Ver_Proyecto-181717?style=for-the-badge&logo=github&logoColor=white" />
        </a>
      </p>
      <p align="center">
        <img src="https://img.shields.io/badge/Docker-2CA5E0?style=flat-square&logo=docker&logoColor=white" />
        <img src="https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazon-aws&logoColor=white" />
        <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=github-actions&logoColor=white" />
        <img src="https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white" />
      </p>
    </td>
    <td width="50%" valign="top">
      <p>Plataforma de gestión de despachos con arquitectura de <strong>microservicios</strong>, contenedorización con Docker y despliegue continuo en AWS EC2.</p>
      <p><strong>Highlights técnicos:</strong></p>
      <ul>
        <li>✅ Multi-Stage Builds (reducción 70% peso imágenes)</li>
        <li>✅ Pipeline CI/CD híbrido en GitHub Actions</li>
        <li>✅ Named Volumes para persistencia transaccional</li>
        <li>✅ Red interna aislada (backend_net)</li>
        <li>✅ Principio de mínimo privilegio (no root)</li>
        <li>✅ Solución al OOM en t2.micro (1GB RAM)</li>
      </ul>
    </td>
  </tr>
</table>

---

## 🔭 Actualmente aprendiendo

- 🔐 **Hardening** y seguridad avanzada en Linux
- 🛡️ Administración de sistemas Linux en producción
- 🐍 **Automatización con Python** y scripting
- ⚡ JavaScript moderno y frameworks
- 🍃 Bases de datos **NoSQL** con MongoDB
- 🐳 Orquestación con **Kubernetes** (próximamente)

---

## 🛠️ Tech Stack

#### Languages  
<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/C%2B%2B-00599C?style=flat-square&logo=c%2B%2B&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-00758F?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white" />
</p>

#### Backend & Frameworks  
<p>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white" />
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" />
</p>

#### DevOps & Cloud  
<p>
  <img src="https://img.shields.io/badge/Docker-2CA5E0?style=flat-square&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker_Compose-2496ED?style=flat-square&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=github-actions&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS_EC2-FF9900?style=flat-square&logo=amazon-aws&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" />
</p>

#### Databases  
<p>
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white" />
</p>

#### Tools  
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/VSCode-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white" />
  <img src="https://img.shields.io/badge/IntelliJ_IDEA-000000?style=flat-square&logo=intellij-idea&logoColor=white" />
  <img src="https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white" />
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white" />
</p>

---

## 📂 Mis Repositorios

Aquí tienes todos mis proyectos públicos y avances:

### 🛒 Ecomarket SPA
🔗 [github.com/emilio-araya/ecomarket-spa](https://github.com/emilio-araya/ecomarket-spa)  
Aplicación de escritorio para la gestión de stock y ventas en **Java** con POO sólida y validaciones estrictas.

### 📊 Programación Bases de Datos PL/SQL
🔗 [github.com/emilio-araya/Programasion-Bases-de-datos-pl-sql](https://github.com/emilio-araya/Programasion-Bases-de-datos-pl-sql)  
Repositorio de ejercicios, procedimientos almacenados, triggers y avances en bases de datos SQL / PL-SQL.

### 💧 Riego Automático IoT
🔗 [github.com/emilio-araya/riego-automatico-arduino](https://github.com/emilio-araya/riego-automatico-arduino)  
Sistema embebido de automatización de riego con sensores y Arduino programado en **C++**.

---

## 📊 GitHub Stats


<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=emilio-araya&theme=tokyonight" />
</p>

---

## 🌐 Connect with me

<p align="center">
  Estoy abierto a colaborar, discutir ideas o simplemente charlar de tecnología.
</p>

<p align="center">
  <a href="mailto:emiliolokillo611@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/emilio-araya" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <em>"El conocimiento no se hereda, se construye línea por línea."</em>
</p>
